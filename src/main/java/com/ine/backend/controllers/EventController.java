...
public class EventController {
	...
	public ResponseEntity<ApiResponseDto<EventResponseDto>> createEvent(@Valid @RequestBody EventRequestDto eventRequest) {
		try {
			EventResponseDto newEvent = eventService.createEvent(eventRequest);
			ApiResponseDto<EventResponseDto> response = ApiResponseDto.<EventResponseDto>builder()
				.isSuccess(true)
			...
			ApiResponseDto<EventResponseDto> response = ApiResponseDto.<EventResponseDto>builder()
				.isSuccess(false)
				.message("Error occurred while creating event: " + e.getMessage())
				.response(null)
				.build();
	...
	public ResponseEntity<ApiResponseDto<List<EventResponseDto>>> getAllEvents() {
		try {
			...
			ApiResponseDto<List<EventResponseDto>> response = ApiResponseDto.<List<EventResponseDto>>builder()
				.isSuccess(false)
				.message("Error occurred while retrieving events: " + e.getMessage())
				.response(null)
				.build();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
		}
	...
	public ResponseEntity<ApiResponseDto<EventResponseDto>> getEventById(@PathVariable Long id) {
		try {
			...
			ApiResponseDto<EventResponseDto> response = ApiResponseDto.<EventResponseDto>builder()
				.isSuccess(false)
				.message("Error occurred while retrieving event: " + e.getMessage())
				.response(null)
				.build();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
		}
	...
	public ResponseEntity<ApiResponseDto<EventResponseDto>> updateEvent(
		...
		try {
			...
			ApiResponseDto<EventResponseDto> response = ApiResponseDto.<EventResponseDto>builder()
				.isSuccess(false)
				.message("Error occurred while updating event: " + e.getMessage())
				.response(null)
				.build();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
		}
